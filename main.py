"""
main.py - Streamlit UI layer for MindEase Mental Health Support Chatbot.
"""
...
.token-info { font-size: 0.75rem; color: #9dbdb6; text-align: right; margin-top: 0.3rem; }
.crisis-banner {
...
def _init_state():
    ...
    if "gemini_client" not in st.session_state:
        ...
        except (EnvironmentError, ValueError) as e:   # ✅ catches both
...
if st.session_state.get("client_error"):
    st.error(f"⚠️ Configuration Error: {st.session_state.client_error}")
    st.info("Set your `GEMINI_API_KEY` environment variable and restart the app.")
    st.stop()
...
for role, text, token_info in st.session_state.display_history:
    if role == "user":
        ...
        if token_info:
            st.markdown(f'<div class="token-info">Tokens used: {token_info["total_tokens"]}</div>', unsafe_allow_html=True)
...
if send_btn and user_input.strip():
    ...
    st.session_state.display_history.append(("user", message, None))
    ...
    with st.spinner("MindEase is thinking…"):
        ...
        response_text, usage = client.get_response(
            chat_history=sm.get_history(),
            user_message=message,
        )

    sm.add_model_message(response_text)
    st.session_state.display_history.append(("bot", response_text, usage))
    st.rerun()
